        ...
        try:
            if (gameState.board[pos1][pos2].endswith("k")):
                if ((gameState.board[r - plus][c + plus] == "--" and
                     gameState.board[r - plus - 1][c + plus + 1] == "--") and gameState.board[pos1][pos2]):
                    return True
    ...
    for r in range(DIMENSIONS):
        for c in range(DIMENSIONS):
            if gameState.board[r][c] == "bl":
                ...
                if ((c + 1 < DIMENSIONS) and (r + 1 < DIMENSIONS) and (gameState.board[r + 1][c + 1] == "--")):
                    possibleMoves.append(r + 1)
                    possibleMoves.append(c + 1)

                if ((r + 1 < DIMENSIONS) and (gameState.board[r + 1][c - 1] == "--") and (c - 1 >= 0)):
                    possibleMoves.append(r + 1)
                    possibleMoves.append(c - 1)
