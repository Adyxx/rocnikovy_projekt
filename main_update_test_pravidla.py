                        ...
                        if (gameState.board[pos1][pos2] != "--"): #does 1. position have piece?
                            if (gameState.board[row][col] == "--"): # is 2. position empty?
                                ...
                                if move == 1: # if it's White's turn
                                    ...
                                    for s in range(DIMENSTIONS - 1):
                                        for r in range(DIMENSTIONS - 1):
                                            if ((gameState.board[s][r] != "--") and (
                                                    (gameState.board[s + 1][r + 1] != "--") or (
                                                    gameState.board[s + 1][r - 1] != "--") or (
                                                            gameState.board[s - 1][r + 1] != "--") or (
                                                            gameState.board[s - 1][r - 1] != "--")) and (
                                                    (gameState.board[s + 2][r + 2] == "--") or (
                                                    gameState.board[s + 2][r - 2] == "--") or (
                                                            gameState.board[s - 2][r + 2] == "--") or (
                                                            gameState.board[s - 2][r - 2] == "--"))):
                                                print("Musíš brát")
